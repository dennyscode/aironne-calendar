export var Aironne = (function () {

    console.log("aironne-cal v.1.0.2")

    let options = {
        "date": new Date(),
        "view": "month",
        "weekday_view": "number" // or "weekday"
    }

    let date = new Date();
    let calendar_nextbutton;
    let calendar_showNextMonth;
    ...
    function first_wday_of_month(input_date) {
        ...
        if (input_date === undefined) {
            firstweekday = new Date(); //Todo: Replace with Node´s Date
        } else {
    ...
    function first_day_of_month(input_date) {
        if (input_date === undefined) {
            let input_date = new Date(); //Todo: Replace with Node´s Date
    ...
    function weeks_in_a_month(input_date) {
        if (input_date == undefined) {
            let input_date = calendar_convert_date_to_attribute(new Date); //Todo: Replace with Node´s Date
        }
        ...
        let number_of_weeks =  Math.ceil((d1 + d2) / 7);
    ...
    function calendar_week(input_date, node) {
        console.log("Calendar Week:")
        console.log(input_date)
        let yearStart, weekNo;
        // Copy date so don't modify original
        console.log(input_date === undefined)
        if (input_date === undefined) {
            let input_date = new Date(); //Todo: Replace with Node´s Date
            ...
            input_date.setUTCDate(input_date.getUTCDate() + 4 - (input_date.getUTCDay()||7));
            yearStart = new Date(Date.UTC(input_date.getUTCFullYear(),0,1));
            weekNo = Math.ceil(( ( (input_date - yearStart) / 86400000) + 1)/7);
    ...
    let capitalize = function(str1) {
        return str1.charAt(0).toUpperCase() + str1.slice(1);
    }

    let format_two_digit = function(str) {
        let output = "0" + (str)
    ...
    function calendar_switch_month(option, input_date) {
        console.log("Calendar Switch Month:")
        console.log(input_date)
        if (option == "next") {
    ...
    function show_calendar(options) {
        ...
        if (set_weekday_view === "number") {
            ...
            let node_headercontainer = document.createElement("div");
            node_headercontainer.setAttribute("class", "node_header");
    ...
    function show_view_4days(input_date) {
        ...
        return {
            Options: options,
            testFunction: function() { console.log("TESTFUNCTION WUHU!")}
        }
    }
})()
